...
export default function BookAppointment({ navigation, route }) {
  ...
  useEffect(() => {
    api
      .request("getAvailableDate", "POST", { doctorId: doctorId })
      .then((response) => {
  ...
  const getMarkedDate = () => {
    ...
    availableDate.map((item) => {
      dateObject[item] = { disabled: false };
    });
      ...
      <View style={styles.scheduleContainer}>
        ...
        <View>
          {item.status == "AVAILABLE" ? (
            ...
            <TouchableOpacity disabled style={styles.bookedButton}>
