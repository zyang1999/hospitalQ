...
export default function AppointmentDetails({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    getUserRole();
    api
      .request("getAppointmentDetails", "POST", {
  ...
  const removeAppointment = (reason) => {
    ...
    api
      .request("deleteAppointment", "POST", {
        id: appointment.id,
        feedback: reason,
      })
      .then((response) => {
  ...
  const completeAppointment = () => {
    api
      .request("completeAppointment", "POST", { id: appointment.id })
      .then((response) => {
  ...
  const submitFeedback = (feedback) => {
    api
      .request("storeAppointmentFeedback", "POST", {
        appointmentId: appointment.id,
        feedback: feedback,
      })
      .then((response) => {
        ...
        {appointment.status == "AVAILABLE" ? (
          ...
          <View>
            <Text styles={globalStyles.h5}>
              There is a patient who already booked this appointment! Please
              provide your reason in order to remove this appointment.
            </Text>
            <TextInput
              multiline
              numberOfLines={4}
  ...
  const Button = () => {
    if (userRole == "PATIENT") {
      if (appointment.status == "COMPLETED") {
        ...
        return <Reason appointment={appointment} />;
      } else {
        return <View></View>;
      }
    } else {
        ...
        if (checkDate(appointment.date_string) == "today") {
          ...
          return (
            <Text style={{ margin: 10 }}>
              This appointment can't be deleted because it was in the past.
            </Text>
          );
        }
      }
            ...
            >
              <View>
                <View>
                  ...
                  <Text style={styles.details}>{appointment.date_string}</Text>
                </View>
                <View>
